# 各言語の概要 / Per-language summaries

本フォークの短い概要を、対応する各言語で置いています。README全文の翻訳ではありません。

A short summary of this fork in each language it supports. These are not full translations of the README.

## 東アジア

- [日本語](ja.md) (`ja`)
- [한국어](ko.md) (`ko`)
- [Монгол](mn.md) (`mn`)
- [廣東話](yue.md) (`yue`)
- [简体中文](zh-Hans.md) (`zh-Hans`)
- [繁體中文](zh-Hant.md) (`zh-Hant`)

## ヨーロッパ

- [Azərbaycan dili](az.md) (`az`)
- [Беларуская](be.md) (`be`)
- [Български](bg.md) (`bg`)
- [Bosanski](bs.md) (`bs`)
- [Català](ca.md) (`ca`)
- [Crnogorski](cnr.md) (`cnr`)
- [Čeština](cs.md) (`cs`)
- [Dansk](da.md) (`da`)
- [Deutsch](de.md) (`de`)
- [Ελληνικά](el.md) (`el`)
- [English](en.md) (`en`)
- [Español](es.md) (`es`)
- [Eesti](et.md) (`et`)
- [Suomi](fi.md) (`fi`)
- [Français](fr.md) (`fr`)
- [Gaeilge](ga.md) (`ga`)
- [Hrvatski](hr.md) (`hr`)
- [Magyar](hu.md) (`hu`)
- [Հայերեն](hy.md) (`hy`)
- [Íslenska](is.md) (`is`)
- [Italiano](it.md) (`it`)
- [ქართული](ka.md) (`ka`)
- [Lëtzebuergesch](lb.md) (`lb`)
- [Lietuvių](lt.md) (`lt`)
- [Latviešu](lv.md) (`lv`)
- [Македонски](mk.md) (`mk`)
- [Malti](mt.md) (`mt`)
- [Norsk bokmål](nb.md) (`nb`)
- [Nederlands](nl.md) (`nl`)
- [Norsk nynorsk](nn.md) (`nn`)
- [Polski](pl.md) (`pl`)
- [Português](pt.md) (`pt`)
- [Rumantsch](rm.md) (`rm`)
- [Română](ro.md) (`ro`)
- [Русский](ru.md) (`ru`)
- [Slovenčina](sk.md) (`sk`)
- [Slovenščina](sl.md) (`sl`)
- [Shqip](sq.md) (`sq`)
- [Српски](sr.md) (`sr`)
- [Svenska](sv.md) (`sv`)
- [Türkçe](tr.md) (`tr`)
- [Українська](uk.md) (`uk`)

## 西アジア・中東

- [العربية](ar.md) (`ar`)
- [کوردیی ناوەندی](ckb.md) (`ckb`)
- [فارسی](fa.md) (`fa`)
- [עברית](he.md) (`he`)

## 南アジア

- [বাংলা](bn.md) (`bn`)
- [ગુજરાતી](gu.md) (`gu`)
- [हिन्दी](hi.md) (`hi`)
- [ಕನ್ನಡ](kn.md) (`kn`)
- [മലയാളം](ml.md) (`ml`)
- [मराठी](mr.md) (`mr`)
- [नेपाली](ne.md) (`ne`)
- [ଓଡ଼ିଆ](or.md) (`or`)
- [ਪੰਜਾਬੀ](pa.md) (`pa`)
- [සිංහල](si.md) (`si`)
- [தமிழ்](ta.md) (`ta`)
- [తెలుగు](te.md) (`te`)
- [اردو](ur.md) (`ur`)

## 東南アジア

- [Filipino](fil.md) (`fil`)
- [Bahasa Indonesia](id.md) (`id`)
- [ខ្មែរ](km.md) (`km`)
- [ລາວ](lo.md) (`lo`)
- [Bahasa Melayu](ms.md) (`ms`)
- [မြန်မာ](my.md) (`my`)
- [ไทย](th.md) (`th`)
- [Tiếng Việt](vi.md) (`vi`)
