  ...
  <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
    <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-800/50 flex items-center justify-between">
      ...
      <button
        ...
        :title="isRefreshing ? t('emailList.refreshing') : t('emailList.refreshEmails')"
      >
          ...
          <div class="flex items-start justify-between gap-4">
            ...
            <div class="flex-shrink-0 flex items-center gap-2">
              ...
              <div class="flex sm:hidden sm:group-hover:flex items-center gap-1">
                ...
                <button @click.prevent="toggleReadStatus(email)" class="p-2 text-gray-400 hover:text-indigo-600 dark:text-gray-500 dark:hover:text-indigo-400 rounded-lg hover:bg-indigo-50 dark:hover:bg-gray-700/50 transition-all duration-200" :title="email.read ? t('emailList.markUnread') : t('emailList.markRead')">
    ...
    <!-- Scroll sentinel: reaching it pulls in the next page. -->
    <div v-if="emails.length > 0 && hasMore" ref="loadMoreTrigger" class="py-6 flex items-center justify-center text-sm text-gray-500 dark:text-gray-400 gap-2">
    ...
    <div v-if="emails.length === 0" class="p-16 text-center">
      ...
      <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-2">{{ t("emailList.emptyTitle") }}</h2>
      <p class="text-gray-600 dark:text-gray-400">{{ t("emailList.emptyMessage") }}</p>
    </div>
  </div>
</template>
...
const defaultFolderNameKeys: Record<string, string> = {
	inbox: "sidebar.inbox",
	sent: "sidebar.sent",
	draft: "sidebar.draft",
...
const folderName = computed(() => {
	const key = defaultFolderNameKeys[folderId.value.toLowerCase()];
	if (key) return t(key);
...
/**
 * Pulls in the next page when the end of the list comes into view.
 *
 * After each page the sentinel is re-observed: a fresh observe() reports the
 * element's current state, so a sentinel still on screen (short page, tall
...
onMounted(() => {
	observer = new IntersectionObserver(
		...
		// Start fetching just before the sentinel is reached, so the next page
