...
画面は **73言語** で使えます。ログイン画面を含むどの画面からでも右上のセレクタで切り替えられ、選んだ言語はブラウザの localStorage に残ります。
...
対象は、東アジア6言語、ヨーロッパ42言語、西アジア・中東4言語、南アジア13言語、東南アジア8言語です。「どの言語が重要か」という判断ではなく、地域ごとに線を引いて機械的に決めています。後から見て検証できる基準のほうが、好みで選んだ一覧より維持しやすいためです。

**西アジア・中東**は、その地域の国家公用語のうち**ヨーロッパに既に入っているもの**（欧州評議会加盟国であるアルメニア・アゼルバイジャン・ジョージア・トルコ・キプロス）を除いた残りです。残るのはアラビア語・ヘブライ語・ペルシア語・クルド語の4つ。ペルシア語をここに置いたのは、国連の統計上の区分では南アジアに入るものの、それ以外のどの読み方でもこの地域に属し、本フォークの「南アジア」はインド亜半島を指しているためです。

クルド語が線の内側に入る根拠は、**イラク憲法第4条**がアラビア語と並ぶ公用語と定めている、その一点です。したがって採るべきはイラクおよびクルディスタン地域政府の書記標準である**中央クルド語（ソラニー、`ckb`）**で、ペルシア・アラビア文字・RTLです。話者数で上回る**クルマンジー（北部クルド語 `ku`、ラテン文字、LTR）は、どの国の公用語でもない**ため、バスク語・ウェールズ語・フェロー語をヨーロッパから外したのと同じ規則で線の外側になります。文字も書字方向も違うので1つのカタログでは賄えません（`zh-Hans`/`zh-Hant`、`nb`/`nn` と同じ構図ですが、国家公用語なのは片方だけです）。
...
| 西アジア・中東 | العربية, کوردیی ناوەندی, فارسی, עברית |
...
各言語の短い概要を [`docs/readme/`](docs/readme/README.md) に1言語1ファイルで置いています。README全文の翻訳ではありません。全文を73言語ぶん維持するのは現実的ではなく、古びた訳文が並ぶくらいなら短くても正しいほうがよいと判断しました。
...
- **Worker が送信するメール**（`packages/worker/src/mail-templates.ts`：パスワード再設定・メールアドレス変更の確認）も**73言語すべてに対応しています**。2通で文面が同じ2文（リンクのコピー案内と有効期限）は `EMAIL_CHANGE` 側で書かず `PASSWORD_RESET` から取っているので、この半分は原理的にズレません。

  `MAIL_LOCALES` は**UIの73言語と同じ集合でなければなりません**。ダッシュボードは表示中のロケールをそのまま送り、Worker 側は `z.enum(MAIL_LOCALES)` で検証するため、リストから漏れたコードは日本語にフォールバックするのではなく **400 で弾かれ、メールが1通も送られません**。しかも画面には「アカウントがあればリンクを送りました」と出るので気づけません（実際に、UIを72言語に増やしたときこのリストが3言語のままで、66言語でパスワード再設定が死んでいました）。`packages/dashboard/src/locales/mailLocales.test.ts` が両者の集合一致を検査します。
...
> 上流には「これをコピーして自分のWorkerを作る」ための `template/` があり、その `package.json` は npm の `email-explorer@^1.0.4` を入れる作りでした。それは**上流のパッケージ**であって、本フォークの成果（73言語対応・添付・バックアップ・迷惑メール判定など）は一切含まれません。フォークして配布する形にした以上、そこにあること自体が誤解を招くので外しました。
...
外部への送信は Resend を通ります。その API キーは **管理画面（`/admin`）で設定**します。以前は GitHub Actions のシークレットから毎デプロイ `wrangler secret put` で流し込んでいましたが、交換のたびに GitHub を開いて再デプロイする必要がありました。

- **鍵は画面に返しません。** API が答えるのは「設定済みかどうか」と「どちらの出所か」だけです。セッションを盗まれても鍵は盗まれません。
- **管理者専用**です。鍵を持つ者は、この配備が送る全メールの経路を決められます。管理者以外のセッションには 403、セッション無しには 401。
- **公開エンドポイント `GET /api/v1/settings` には載せません。** そこはログイン前に読めるので、送信が設定済みかどうかすら教える理由がありません。
- **デプロイ時の環境変数はフォールバックとして残します。** この画面を導入したデプロイでメールが止まらないようにするためと、保存した鍵が誤っていたときの戻り道のためです。保存した鍵が常に優先されます。
- 空文字で保存すると保存済みの鍵を消し、環境変数側があればそちらに戻ります。空文字を鍵として保存はしません（`Bearer ` を送って全メールが理由不明に失敗するため）。

**この変更で弱くなる点を明記します。** Worker secret は書き込み専用で、Cloudflare の管理画面からも読み出せません。R2 に置くと平文でバケットに載り、**Cloudflare アカウントを持つ者は読めます**。ただしそれは自動バックアップの設計時点で既に「守れない」と明記した線の外側です。その線の内側 ── このアプリの管理者パスワードが漏れた場合 ── では実質中立で、そもそもその人物は全メールを読めて任意のメールボックスとして送れます。

`deploy.yml` から `RESEND_API_KEY` を流し込む手順は削除しました。既にWorkerに設定済みのシークレットはそのまま残るので、手順を消しても送信は止まりません。
...
**下げられます。** 自動バックアップの保存数は「下げられない」ことがその機能の要点でしたが、ここでは事情が違います ── 消えるのは迷惑メールで、しかもバックアップが先に走っています。

下限は 1 日で、0 日は選べません。「毎晩、迷惑メールフォルダを空にする」は保持方針ではなく、その朝に誤判定で入ったメールを誰も出社する前に失う方法です。上限は 365 日、未設定・空欄は 30 日です。**空欄を 0 と読まないのが要点で**、`Number("")` も `Number(null)` も 0 なので、素直に書くと「入力欄を消して保存したら 1 日」になります（`normalizeRetentionDays`）。
