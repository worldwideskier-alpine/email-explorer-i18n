...
- 何度でも再実行可能ですが、**重複インポート防止機能はありません**。`imap_folders` に
  **既に移行済みのフォルダを再度含めない**よう注意してください。再実行する場合は
  `LOLIPOP_IMAP_SINCE` で未取り込み分だけに絞り込むか、フォルダ単位で分けて実行してください。
...
- 内容を確認し、問題なければ `apply: true` で再実行すると実際に重複分を削除します（1組につき1件を残して削除）

件名・送信元・宛先・日時が完全一致する組み合わせを重複と判定します。何件重複していても
（2件でも3件以上でも）、既読・スター付きの1件（無ければ最初の1件）だけを残して他を削除します。
...
- ロリポップから1通ずつ取得し、件名・送信元・宛先・日時が完全一致する既存メールが見つかれば、
  そのメールに元データだけを追加で紐付けます（既読・スター等の状態はそのまま）。
- 一致する既存メールが無ければ、新規メールとして通常通りインポートします（未移行分の取りこぼし救済）。
- 2件以上に一致してしまう場合（既存の重複が残っている等）は自動処理せず「要確認」として表示します。
- `dry_run: true` でまず内容を確認し、問題なければ `dry_run: false` で実行してください。
- ローカル実行する場合は `npm run backfill-source`。
